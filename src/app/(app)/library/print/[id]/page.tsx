...
export default async function PrintArticlePage({
  ...
  const [{ data: articleData }, { data: notesData }] = await Promise.all([
    ...
    supabase
      .from("notes")
      .select("id, title, content, source_page, updated_at, note_tags(tags(name))")
      .eq("source_id", id)
      ...
      <div className="a4-sheet">
        ...
        <section className="mt-6">
          ...
          <div className="mt-2 space-y-5">
            {notes.map((n) => {
              ...
              return (
                ...
                >
                  ...
                  {n.source_page && (
                    <p className="text-xs text-stone-500">s. {n.source_page}</p>
                  )}
            ...
            {notes.length === 0 && (
              <p className="text-sm text-stone-500">Bu makaleye ait not yok.</p>
            )}
          </div>
        </section>
