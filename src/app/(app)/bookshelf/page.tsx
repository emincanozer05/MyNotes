    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      {books.length > 0 ? (
        <div className="stagger grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {books.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className="glass-card group relative flex flex-col rounded-xl p-2.5"
              >
                <form action={deleteBook} className="absolute right-2 top-2 z-10">
                  <input type="hidden" name="id" value={b.id} />
                  <button
                    title="Kitabı sil"
                    className="flex h-6 w-6 items-center justify-center rounded-full bg-black/40 text-sm text-white opacity-0 transition-opacity hover:bg-rose-500 group-hover:opacity-100"
                  >
                    ×
                  </button>
                </form>
                ...
                <Link href={`/bookshelf/${b.id}`} className="block">
                  {b.cover_url ? (
                    ...
                    <img
                      src={b.cover_url}
                      alt={`${b.title} kapağı`}
                      className="aspect-[2/3] w-full rounded-md border border-[var(--border)] object-cover shadow-md transition-transform group-hover:scale-[1.03]"
                    />
                  ) : (
                    <div
                      className={`flex aspect-[2/3] w-full items-center justify-center rounded-md bg-gradient-to-br p-2 text-center shadow-md transition-transform group-hover:scale-[1.03] ${spineColor(b.title)}`}
                    >
                ...
                <Link href={`/bookshelf/${b.id}`} className="mt-2 min-w-0">
        ...
        <p className="rounded-2xl border border-dashed border-stone-300 p-10 text-center text-sm text-stone-500 dark:border-stone-700">
          ...
          <span className="font-semibold">+ Kitap Ekle</span> ile ilk kitabınızı
          ekleyin.
        </p>
      )}
    </div>
